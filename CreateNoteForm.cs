...
namespace SecureNotes
{
    ...
    public class CreateNoteForm : Form
    {
        ...
        private void InitializeModernUI(string defaultType, Note toEdit)
        {
            ...
            bottomPanel.SizeChanged += (s, e) => RepositionBottomControls();
            Shown += (s, e) => RepositionBottomControls();
        ...
        private void UpdateAttachmentsPanel()
        {
            ...
            int panelWidth = Math.Max(260, bottomPanel.ClientSize.Width - 60);
        ...
        private void RepositionBottomControls()
        {
            ...
            int padding = 20;
            int y = 10;
            int contentWidth = Math.Max(260, bottomPanel.ClientSize.Width - (padding * 2));

            if (bottomPanel.VerticalScroll.Visible)
            {
                contentWidth = Math.Max(260, contentWidth - SystemInformation.VerticalScrollBarWidth);
            }

            if (imagePreviewPanel.Visible && imagePreviewPanel.Height > 0)
            {
                imagePreviewPanel.Location = new Point(padding, y);
                imagePreviewPanel.Width = contentWidth;
                UpdateAttachmentRowWidths(contentWidth);
            ...
            txtTags.Width = Math.Max(120, contentWidth - 180);
            y += 40;
            ...
            cmbGroup.Width = contentWidth;
            y += 40;

            btnSave.Location = new Point(padding, y);
            btnSave.Width = contentWidth;
            y += btnSave.Height + 20;
        ...
        private void UpdateAttachmentRowWidths(int panelWidth)
        {
            foreach (Control control in imagePreviewPanel.Controls)
            {
                if (!(control is Panel row)) continue;

                row.Width = panelWidth;

                foreach (Control child in row.Controls)
                {
                    if (child is Button removeButton)
                    {
                        removeButton.Location = new Point(panelWidth - removeButton.Width - 6, removeButton.Location.Y);
                    }
                }
            }
        }
